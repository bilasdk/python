...
from ..types.account_get_balance_response import AccountGetBalanceResponse
...
class AccountsResource(SyncAPIResource):
    ...
    def retrieve(
        ...
        *,
        ...
        return self._get(
            path_template("/api/v1/bila/accounts/{id}", id=id),
    ...
    def get_balance(
        ...
        *,
        ...
        if not id:
            raise ValueError(f"Expected a non-empty value for `id` but received {id!r}")
        return self._get(
            path_template("/api/v1/bila/accounts/{id}/balance", id=id),
            options=make_request_options(
                extra_headers=extra_headers, extra_query=extra_query, extra_body=extra_body, timeout=timeout
            ),
